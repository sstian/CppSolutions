...
void flush()
{
	...
	outtextxy(20, 2 * NUM_R*NUM_Y + 2, L"          ");
	outtextxy(20, 2 * NUM_R*NUM_Y + 2, (TCHAR)string);
}
...
void over(bool a)                       //结束游戏
{
	...
	if (a)                               //判断条件
		outtextxy(NUM_X*NUM_R - 20, NUM_Y*NUM_R - 20, L"胜利");      //输出结果
	else
		outtextxy(NUM_X*NUM_R - 20, NUM_Y*NUM_R - 20, L"失败");      //输出结果
	Sleep(2000);
	system("pause");
	exit(0);
}