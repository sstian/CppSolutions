...
// InfoManageDoc.cpp: CInfoManageDoc 类的实现
//
...
// CInfoManageDoc 构造/析构

CInfoManageDoc::CInfoManageDoc() noexcept 
{
...
void CInfoManageDoc::Serialize(CArchive& ar)
{
	...
	//pFrame->GetActiveView()->Serialize(ar);
...
void CInfoManageDoc::DeleteContents()
{
	POSITION pos = GetFirstViewPosition();
	if (pos)
	{
		CInfoManageView* pView = (CInfoManageView*)GetNextView(pos);
...
// 缩略图的支持
void CInfoManageDoc::OnDrawThumbnail(CDC& dc, LPRECT lprcBounds)
{
	// 修改此代码以绘制文档数据
...
// 搜索处理程序的支持
void CInfoManageDoc::InitializeSearchContent()
{
...
void CInfoManageDoc::SetSearchContent(const CString& value)
{
	if (value.IsEmpty())
	{
