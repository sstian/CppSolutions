// CTriangleButton.cpp: 实现文件
//

#include "stdafx.h"
#include "CharsMap.h"
#include "CTriangleButton.h"


// CTriangleButton

IMPLEMENT_DYNAMIC(CTriangleButton, CMFCButton)
...
BEGIN_MESSAGE_MAP(CTriangleButton, CMFCButton)
END_MESSAGE_MAP()
...
void CTriangleButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	case POINT_RIGHT: {
		if (nStyle & BS_FLAT) { //style is flat
			...
			pDC->SelectObject(HighlightPen);
			pDC->MoveTo(1, 2);
			pDC->LineTo(rect.right / 2 - 2, rect.bottom / 2);
			...
			if ((state & ODS_SELECTED)) { //Button is down 
				...
				pDC->LineTo(1, 1);
				pDC->MoveTo(rect.right / 2 - 3, rect.bottom / 2);
				pDC->LineTo(0, 1);
				...
				pDC->LineTo(rect.right / 2, rect.bottom / 2 + 1);
