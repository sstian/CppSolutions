...
#include <afxext.h>
#include "..\CharsMap\CharsMapDoc.h"
#include "..\CharsMap\CharsMapView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CCharsMapView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CCharsMapDoc, (CObject*) m_pDocument->GetContainer());
